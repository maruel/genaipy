...
func NewServer(ctx context.Context, script, cacheDir, logName string, extraArgs []string) (*Server, error) {
	...
	cp, err := openaicompatible.New(ctx, genai.ProviderOptionRemote(s.URL+"/v1/chat/completions"))
	if err != nil {
		return nil, err
	}
	for {
		...
		case <-time.After(100 * time.Millisecond):
			_, err := cp.GenSync(ctx, genai.Messages{genai.NewTextMessage("Say hello. Use only one word.")})
			if err == nil {
				return s, nil
			}
		}
